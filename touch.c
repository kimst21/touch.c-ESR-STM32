...
#define CS_ON								(HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, GPIO_PIN_RESET))
#define CS_OFF 								(HAL_GPIO_WritePin(GPIOB, GPIO_PIN_6, GPIO_PIN_SET))
...
static uint8_t SpiTransfer(uint8_t byte)
{
	...
	(void)HAL_SPI_TransmitReceive(&hspi4, &byte, &result, 1U, 1000U);
